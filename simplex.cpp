...
double * simplex_method(std::vector<double> c, std::vector<double> b, std::vector<std::vector<double>>& a)
{
	using std::cout;
	using std::endl;
	using std::vector;
	...
	if (strategy[0].find("V") != -1)
	{
		...
		for (unsigned int i = {0}; i < x_strategy.size(); ++i)
		{
			cout << "       y" << index[i] << " = " << "V" << index[i] << " * h = " << x_strategy[i] << ";" << endl;
		}
		cout << endl <<  std::setw(25) << "Проверка: " << endl << "       "; // Проверка на то, что сумма вероятностей == 1
		double sum = {0.0};
		for (unsigned int i = {0}; i < x_strategy.size(); ++i)
...
void set_new_data(std::vector<double> &c, std::vector<double> &b, std::vector<std::vector<double>>& a)
{
	...
	c.swap(b); // Меняем с и b, далее меняем c и b в файле lab4.txt
